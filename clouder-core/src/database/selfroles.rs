...
impl SelfRoleConfig {
    pub async fn get_by_id(pool: &SqlitePool, id: i64) -> Result<Option<Self>> {
        let config = sqlx::query_as::<_, Self>("SELECT * FROM selfrole_configs WHERE id = ?")
            .bind(id)
            .fetch_optional(pool)
            .await?;
    ...
    pub async fn update_message_id(&mut self, pool: &SqlitePool, message_id: &str) -> Result<()> {
        sqlx::query(
            r#"
    ...
    pub async fn update_channel_id(&mut self, pool: &SqlitePool, channel_id: &str) -> Result<()> {
        sqlx::query(
            r#"
            UPDATE selfrole_configs
    ...
    pub async fn get_by_message_id(pool: &SqlitePool, message_id: &str) -> Result<Option<Self>> {
        let config =
    ...
    pub async fn get_by_guild(pool: &SqlitePool, guild_id: &str) -> Result<Vec<Self>> {
    ...
    pub async fn get_roles(&self, pool: &SqlitePool) -> Result<Vec<SelfRoleRole>> {
        let roles =
            sqlx::query_as::<_, SelfRoleRole>("SELECT * FROM selfrole_roles WHERE config_id = ?")
                .bind(self.id)
                .fetch_all(pool)
                .await?;
...
impl SelfRoleRole {
    pub async fn create(
        ...
        let role = sqlx::query_as::<_, Self>("SELECT * FROM selfrole_roles WHERE id = ?")
            .bind(result.last_insert_rowid())
            .fetch_one(pool)
            .await?;
...
impl SelfRoleLabel {
    pub async fn get(pool: &SqlitePool, guild_id: &str, role_id: &str) -> Result<Option<Self>> {
        Ok(sqlx::query_as::<_, Self>(
            "SELECT guild_id, role_id, name FROM selfrole_labels WHERE guild_id = ? AND role_id = ?",
        )
        .bind(guild_id)
        .bind(role_id)
        .fetch_optional(pool)
    ...
    pub async fn upsert(
        ...
        sqlx::query(
            r#"
            INSERT INTO selfrole_labels (guild_id, role_id, name)
            VALUES (?, ?, ?)
            ON CONFLICT (guild_id, role_id) DO UPDATE SET name = excluded.name, updated_at = CURRENT_TIMESTAMP
            "#,
        )
        .bind(guild_id)
        .bind(role_id)
    ...
    pub async fn upsert_many(
        ...
        for (role_id, name) in roles {
            Self::upsert(pool, guild_id, role_id, name).await?;
        }
        Ok(())
    }
    pub async fn get_all_for_guild(
        ...
        let labels = sqlx::query_as::<_, Self>(
            "SELECT guild_id, role_id, name FROM selfrole_labels WHERE guild_id = ?",
        )
        .bind(guild_id)
        .fetch_all(pool)
        .await?;
        Ok(labels.into_iter().map(|l| (l.role_id, l.name)).collect())
    }
}
