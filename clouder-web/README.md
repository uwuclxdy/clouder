...
Starts an Axum HTTP server on the address derived from `config.web.api_url`. Receives the same `AppState` used by the bot, giving it access to the database pool and Discord HTTP client.
