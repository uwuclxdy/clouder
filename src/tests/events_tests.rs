...
mod tests {
    ...
    fn test_custom_id_format_validation() {
        ...
        for valid_id in valid_formats {
            ...
            assert_eq!(parts.len(), 3, "Valid ID '{}' should have 3 parts", valid_id);
            assert_eq!(parts[0], "selfrole", "Valid ID '{}' should start with 'selfrole'", valid_id);
            assert!(!parts[1].is_empty(), "Valid ID '{}' should have non-empty config ID", valid_id);
            assert!(parts[1].chars().all(|c| c.is_ascii_digit()), "Valid ID '{}' should have numeric config ID", valid_id);
            assert!(parts[2].starts_with("role"), "Valid ID '{}' should have role part starting with 'role'", valid_id);
        }
        ...
        for invalid_id in invalid_formats {
            ...
            assert!(!is_valid, "Invalid ID '{}' should not pass validation", invalid_id);
        }
    }
    ...
    async fn test_selfrole_cooldown_interaction() {
        ...
        let other_user_cooldown = SelfRoleCooldown::check_cooldown(
            &db,
            "user999",
            "role456",
            "guild789"
        ...
        assert!(!other_user_cooldown, "Different user should not be on cooldown");

        // Test different role - should not be on cooldown
        let other_role_cooldown = SelfRoleCooldown::check_cooldown(
            &db,
            "user123",
            "role999",
            "guild789"
        ...
        assert!(!other_role_cooldown, "Different role should not be on cooldown");
    }
    ...
    async fn test_multiple_configs_deletion() {
        ...
        let _role1 = SelfRoleRole::create(&db, config1_copy.id, "role1", "🎮").await.unwrap();
        let _role2 = SelfRoleRole::create(&db, config2_copy.id, "role2", "🎨").await.unwrap();
        ...
        let deleted = SelfRoleConfig::delete_by_message_id(&db, "msg1").await.unwrap();
        assert!(deleted);
    ...
    async fn test_guild_leave_cleanup() {
        ...
        let _role = SelfRoleRole::create(&db, config.id, "role123", "🎮").await.unwrap();

        // Create cooldowns for users in this guild
        let expires_at = Utc::now() + Duration::hours(1);
        SelfRoleCooldown::create(&db, "user1", "role123", guild_id, expires_at).await.unwrap();
        SelfRoleCooldown::create(&db, "user2", "role456", guild_id, expires_at).await.unwrap();

        // Verify data exists
        let configs = SelfRoleConfig::get_by_guild(&db, guild_id).await.unwrap();
        assert_eq!(configs.len(), 1);

        let cooldown_exists = SelfRoleCooldown::check_cooldown(&db, "user1", "role123", guild_id).await.unwrap();
    ...
    async fn test_concurrent_interaction_handling() {
        ...
        for i in 1..=5 {
            SelfRoleRole::create(&db, config.id, &format!("role{}", i), "🎮").await.unwrap();
        }
        ...
        for user_id in 1..=10 {
            ...
            assert!(!on_cooldown, "User {} should not be on cooldown initially", user_id);
            ...
            assert!(on_cooldown_after, "User {} should be on cooldown after creation", user_id);
        }
    }
    ...
    fn test_error_handling_patterns() {
        ...
        for invalid_id in invalid_ids {
            ...
            assert!(result.is_none(), "Invalid ID '{}' should return None", invalid_id);
        }
        ...
        for (valid_id, expected) in valid_ids {
            ...
            assert!(result.is_some(), "Valid ID '{}' should parse successfully", valid_id);
            let (config_id, role_id) = result.unwrap();
            assert_eq!((config_id, role_id.as_str()), expected, "Parsed values should match expected for '{}'", valid_id);
        }
    }
